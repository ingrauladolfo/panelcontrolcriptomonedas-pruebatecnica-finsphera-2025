...
export const SendMessageModal = ({ user, onClose, onSendMessage }: SendMessageModalProps) => {
    ...
    return (
        <div className="flex justify-center items-center z-1000 p-4 box-border fixed top-0 left-0 size-full bg-[#00000073]">
            <div className="bg-[#d1d1d1] p-5.5 rounded-xl shadow-[0 8px 24px rgba(0, 0, 0, 0.18)] w-full max-w-140 relative box-border max-[480px]:p-4 max-[480px]:rounded-lg max-[480px]:w-full max-[480px]:max-h-screen max-[480px]:h-auto max-[480px]:overflow-auto " role="dialog" aria-modal="true" aria-label={`Enviar mensaje a ${user?.name?.first ?? ''}`}>
                <button
                ...
                <h2 className="m-[0 0 12px 0] text-[20px] text-[#111827] text-left pr-9  max-[480px]:text-[18px] font-semibold">Enviar mensaje a {user?.name?.first} {user?.name?.last}</h2>
                ...
                <form className="flex flex-col gap-2.5" onSubmit={handleSubmit}>
                    ...
                    <textarea
                        id="msg-content"
                        className="w-full py-2.5 px-3 border rounded-lg border-[#111827] bg-[#d1d1d1] text-[14px] text-[#111827] box-border outline-none transition-[box-shadow .12s ease, border-color .12s ease] focus:border-[#3b82f6] focus:shadow-[ 0 0 0 4px rgba(59, 130, 246, 0.08)] max-[480px]:min-h-30"
                        value={content}
